...
var activeObjectId;
var $zoomLevel = 1;
document.getElementById("zoomRangePercent").innerHTML = "50%";
...
function zoom() {
  document.body.addEventListener("mousewheel", zooming);
}
function stopZoom() {
  document.body.removeEventListener("mousewheel", zooming);
}
function zooming(e) {
  var imageEditor2 = document.getElementsByClassName(
    "tui-image-editor-canvas-container"
  );
  imageEditor2[0].style.transform = "scale(" + $zoomLevel + ")";

  if (e.deltaY > 0 && $zoomLevel <= 4) {
    imageEditor2[0].style.transform = `scale(${($zoomLevel += 0.1)})`;
    zoomRange($zoomLevel);
...
function zoomRange(zoomRange) {
  zoomRange *= 100;
  document.getElementById("zoomRangePercent").innerHTML = `${Math.floor(
    zoomRange / 2
  )}%`;
  // document.getElementById("btn-zoomout").disabled = true;
}
...
$inputZoomRange.on("change", function () {
  ...
  var zoomlevel = parseFloat($inputZoomRange.val());
  //imageEditor2[0].style.webkitTransform = "scale("+zoomlevel+")";
  imageEditor2[0].style.transform = "scale(" + zoomlevel + ")";
});
...
// Load sample image
imageEditor
  .loadImageFromURL("img/WhiteBG.jpg", "SampleImage")
